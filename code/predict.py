...
def save_prediction(model, model_root, model_desc, batch_size):
    ...
    for iid in tqdm(iids, desc="predict %r on %r" % (model_root, 'test'), unit="image"):
        print(iid)
        ...
        img_padded = np.array(Image.open('spacenet/Test/'+ iid).crop((-img_extra, -img_extra, full_image_size+img_extra, full_image_size+img_extra)))
